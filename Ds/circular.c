#include <stdio.h> int queue[50],max; int front=-1; int rear=-1;
void enqueue(int element,int max)
{     if(front==-1)
    {         front=0;         rear=0;         queue[rear]=element;
    }
...
void display(int max)
{
    ...
    int choice=1,x;     printf("Enter the size:");
    scanf("%d",&max);
    ...
    case 1:printf("Enter the element which is to be inserted :");
              scanf("%d", &x);               enqueue(x,max);               break;
   case 2: dequeue(max);                break;
  case 3:display(max);
 } }
    return 0; }
